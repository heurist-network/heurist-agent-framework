...
| TokenResolverAgent | Find tokens by address/symbol/name/CoinGecko ID, return normalized profiles and top DEX pools. Pulls extra context (sites/socials/funding/indicators) where available. | • search<br>• profile<br>• pairs | [Source](./agents/token_resolver_agent.py) | CoinGecko, DexScreener, Bitquery (Solana), GMGN/Unifai, Yahoo Finance (optional), Coinsider (optional) |
